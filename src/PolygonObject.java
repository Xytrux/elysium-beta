...
public class DPolygon {
	...
	{		
		...
		for(int i=0; i<x.length; i++)
		{
			...
			newX[i] = (DDD.ScreenSize.getWidth()/2 - Calculator.CalcFocusPos[0]) + CalcPos[0] * Screen.zoom;
			newY[i] = (DDD.ScreenSize.getHeight()/2 - Calculator.CalcFocusPos[1]) + CalcPos[1] * Screen.zoom;			
			if(Calculator.t < 0)
				draw = false;
		}
